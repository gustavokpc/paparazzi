...
void nn_control(const float *state, float *control) {
    ...
    float dt = CFC_TIMESPAN;
    for (int i=0;i<HIDDEN_SIZE;++i) { float f1=tanhf(ff1[i]); float f2=tanhf(ff2[i]); float t=sigmoidf_local(ta[i] * dt + tb[i]); nn_hidden[i]=f1*(1.0f-t)+t*f2; }
